    ...
    public void addOnClickListeners() {
        ...
        finishButton.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (activity.startsWith("com.MainActivity")) {
                    facebookUser = true;
                    Session session;
                    session = ((MainActivity) getActivity()).getFacebookSession();
...
/*
    private int getUserID() {
        ...
        String url = MainActivity.SERVER_URL + "/usrid.py?fname=" + registerData.get(0) + "&lat=" + latlon[0] + "&lon=" + latlon[1];
    ...
    public class Read extends AsyncTask<Void, Integer, Boolean> {
        ...
        protected Boolean doInBackground(Void... params) {
            ...
            try {
                ...
                jsonInstArr = jp.getJsonArray(MainActivity.SERVER_URL + "/institution.py");
                if(jsonInstArr == null){
                ...
                jsonDepartmentArr = jp.getJsonArray(MainActivity.SERVER_URL + "/department.py");
                for (int i = 0; i < jsonDepartmentArr.length(); i++) {
                    departmentObjects.add(new Department(jsonDepartmentArr.getJSONObject(i)));
                }

                jsonStudArr = jp.getJsonArray(MainActivity.SERVER_URL + "/study.py?q=getAllStudies");
                for (int i = 0; i < jsonStudArr.length(); i++) {
                    studyObjects.add(new Study(jsonStudArr.getJSONObject(i)));
                }
                jsonUserArr = jp.getJsonArray(MainActivity.SERVER_URL + "/usr.py?q=usr");
